...
```{r make-circles}
circles <- tribble(~x, ~y, ~shape, ~color, ~radius, ~layer_order, ~color_cat, ~grouping,
...
semicircle_fill <- bind_rows(
  ...
  (make_half_circle(9, c(63.5, 43), 6.5, start = 0.85, end = 0.25, color = "#EEEBF1", color_cat = "white") %>%
     add_row(id = 9, x = 63, y = 42.5, layer_order = 1, color = "#EEEBF1", color_cat = "white")),
  (make_half_circle(10, c(55.5, 44), 5.3, start = 1, end = 0, color = "#E2D9D6", color_cat = "white") %>%
     add_row(id = 10, x = 56, y = 41, layer_order = 1, color = "#E2D9D6", color_cat = "white"))
  ) %>%
  mutate(alpha = 1,
         grouping = id)
...
```{r make-plot}
for (df in c(quads_l, circles_l, lines_l, semicircle_fill_l, semicircle_stroke_color_l, semicircle_stroke_l,
            triangles_l)){
  df <- add_noise(df)
}
...
woo <- quads %>%
  group_by(grouping) %>%
  mutate(noise_h = rnorm(1, 0, magnitude),
         noise_v = rnorm(1, 0, magnitude)) %>%
  ungroup() %>%
  mutate(across(contains("x"), ~ typeof(x)),
         across(contains("y"), ~ typeof(x)))

woo <- add_noise(quads_l)
...
,
  ...
  make_half_circle(18, c(51.5, 66), 17, start = 0.05, end = -0.6, color = "red", color_cat = "black")) %>%
    add_row(id = 18, x = 49, y = 58, layer_order = 1, color = "red", color_cat = "black"))
